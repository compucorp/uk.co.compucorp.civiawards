...
/**
 * @return CRM_CiviAwards_Upgrader
 */
function _civiawards_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/CiviAwards/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_CiviAwards_Upgrader_Base::instance();
  }
}
...
function _civiawards_civix_find_files($dir, $pattern) {
  ...
  while (!empty($todos)) {
    ...
    foreach (_civiawards_civix_glob("$subdir/$pattern") as $match) {
      if (!is_dir($match)) {
        $result[] = $match;
      }
    }
...
function _civiawards_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_civiawards_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _civiawards_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _civiawards_civix_glob(__DIR__ . '/ang/*.ang.php');
...
function _civiawards_civix_fixNavigationMenu(&$nodes) {
  ...
  _civiawards_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _civiawards_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _civiawards_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
